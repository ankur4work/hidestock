...
// Shop metafield used by the storefront extension to read config without an API call.
export const METAFIELD_NAMESPACE = "hidestock";
...
function toStorefrontShape(settings) {
  return {
...
export async function getSettingsForStorefront(shop) {
  ...
  return toStorefrontShape(settings);
}
...
export async function syncSettingsToStorefront(admin, settings) {
  ...
  const value = JSON.stringify(toStorefrontShape(settings));
