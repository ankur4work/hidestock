(function() {
  'use strict';
  const CONFIG_ID = 'hidestock-config';
  const APP_PREFIX = 'hidestock';
  let config = null;
  ...
  function hideBtn(el) {
    ...
    el.classList.add(APP_PREFIX + '-hidden');
    el.dataset.hidden = '1';
  }
  ...
  function processCards() {
    ...
    for (var i = 0; i < cards.length; i++) {
      var card = cards[i];
      if (card.dataset.hidestockProcessed) continue;
      card.dataset.hidestockProcessed = '1';
